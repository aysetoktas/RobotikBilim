function GetLessonByModuleId(mdlId, langId) {
    $('#kt_subheader').remove();
    $('.content').remove();
    $('#homeLogo').remove();
    ...
    var educationName = '';
    var eduLogo = '';
    $.get('/Module/GetLessonByModuleId/?moduleId=' + mdlId + '&&languageId=' + langId, function (data) {
...
function GetEnLessonByModuleId(mdlId, langId) {
    $('#kt_subheader').remove();
    $('.content').remove();
    $('#homeLogo').remove();
    document.getElementById("EduImageDiv").style.display = "none";
    ...
    $.get('/Module/GetLessonByModuleId/?moduleId=' + mdlId + '&&languageId=' + langId, function (data) {
        for (let prop of data) {
            if (counter < 2) {
                ...
                widgetHead = '<div class="kt-widget__head"><div class="kt-widget__media"><img class="" src="' + prop['Logo'] + '" style="width:100%; padding-bottom:10px" alt="image"></div></div>';
                widgetBody = '<div class="kt-widget__body" style="text-align: center;"><div class="kt-widget__contact" style=""><h5 style="color:#616682;">' + prop['Name'] + '</h5></div><h6>' + prop['Code'] + '</h6></div>';
                widgetFooter = '<div class="embed-responsive kt-widget__footer"><a onclick="GetLesson(' + prop['ID'] + ')" type="button" id="' + prop['ID'] + '" class="btn btn-label-danger btn-lg btn-upper" style="">Join Lesson</a></div>';
                endPortletBody = '</div></div></div></div>';
                lessonList += beginPortletBody + widgetHead + widgetBody + widgetFooter + endPortletBody;
            }
            else {
                ...
                widgetHead = '<div class="kt-widget__head"><div class="kt-widget__media"><img class="" src="' + prop['Logo'] + '" style="width:100%; padding-bottom:10px" alt="image"></div></div>';
                widgetBody = '<div class="kt-widget__body" style="text-align: center;"><div class="kt-widget__contact" style=""><h5 style="color:#616682;">' + prop['Name'] + '</h5></div><h6>' + prop['Code'] + '</h6></div>';
                widgetFooter = '<div class="embed-responsive kt-widget__footer"><a onclick="GetLesson(' + prop['ID'] + ')" type="button" id="' + prop['ID'] + '" class="btn btn-label-danger btn-lg btn-upper ' + prop['Class'] + '" style="">Join Lesson</a></div>';
                endPortletBody = '</div></div></div></div>';
                lessonList += beginPortletBody + widgetHead + widgetBody + widgetFooter + endPortletBody;
            }
            counter++;
            subHeader = '<div class="kt-subheader kt-grid__item" id="kt_subheader"><div class="kt-subheader__main"><h3 class="kt-subheader__title">' + prop["EnEducation"] + '</h3><span class="kt-subheader__separator kt-subheader__separator--v"></span><span class="kt-subheader__desc">' + prop["EnLevel"] + ' ' + prop["Hour"] + ' Hour </span></div></div>';
        }
        page = subHeader + beginPage + lessonList + endPage;
